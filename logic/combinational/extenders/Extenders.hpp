...
/**
 * @brief Zero Extender
 * Extends an InWidth bus to an OutWidth bus by padding higher bits with LOW (0).
 */
...
/**
 * @brief Sign Extender
 * Extends an InWidth signed bus to an OutWidth bus by replicating the MSB sign bit.
 */
template <std::size_t InWidth, std::size_t OutWidth>
class SignExtender : public Component
{
    static_assert(OutWidth >= InWidth, "OutWidth must be greater than or equal to InWidth.");

public:
    SignExtender(Bus<InWidth>& in, Bus<OutWidth>& out)
        : m_in(in), m_out(out)
    {
    }
    ...
    void evaluate() noexcept override
    {
        LogicState sign_bit = m_in[InWidth - 1].read();
